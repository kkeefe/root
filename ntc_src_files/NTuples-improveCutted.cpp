...
// the 50% of the total height of the pulse. Applies cutts of energy, after the energy cali-
// bration. Se deben intercambiar los canales 1 y 2 para las pruebas con EJ276.
//
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
cout << argv[1]<< endl;
  if(argc!=5) {
    ...
    cerr << "Usage:\n   "<<argv[3]<<" change factor value. Mean obtained from the energy calibration"<<endl;
    cerr << "Usage:\n   "<<argv[4]<<" relative calibration factor. C-value obatined from the energy calibration"<<endl;
    exit(1);
  }
...
//string nombre = argv[1];
  ...
  string energyFactorStr = argv[3];
  float energyFactor = strtof((energyFactorStr).c_str(),0);
  string cValueStr = argv[4];
  float cValue = strtof((cValueStr).c_str(),0);


  cout<<energyFactor<<" "<<cValue<<endl;
//gROOT->SetBatch();
TApplication app("app",0,0);

gROOT->ProcessLine("#include <vector>");
// string filesList = "/home/kevinkeefegalindo/Documents/ntc_data/rootFiles/Sr90-2018.06.05.Sr90.2/*";
// string filesList = "/home/kevinkeefe/Documents/ntc_data/Sr90BigFileTest/Data/Sr90-2018.06.05.Sr90.2/*";
...
//==============================================================
 ...
 Float_t amplVar1, amplVar1Cal, amplVar2, logAmplitudes, logAmplitudesInv, limitEnergy1, limitEnergy2, Energy1=300, Energy2=400, SrEnergy=477;
 ...
 limitEnergy1=Energy1*energyFactor/SrEnergy;
 limitEnergy2=Energy2*energyFactor/SrEnergy;
...
//==============================================================
 ...
 if(i==0){eventNumberAnt_=eventNumber_;}
         ...
         deltaT1 = (timeCh0-timeCh2);//*0.196;//.196 corresponds to 5.1 Gsamples
         deltaT2 = (timeCh2-timeCh3);//*0.196;//.196 corresponds to 5.1 Gsamples
         amplVar1 = TMath::Sqrt(max0*max2);
         amplVar2 = TMath::Sqrt(max2*max3);
         amplVar1Cal = TMath::Sqrt(max0*max2*cValue);
         R12 = max0/max1;
         R21 = max1/max0;
         RS12 = max0/max3;
         RS21 = max3/max2;
         logAmplitudes = TMath::Log(max0/max3);
        ...
        if (deltaT1<-20){
         ...
         if ((deltaT1<100)&&(deltaT1>-100)&&(deltaT2<100)&&(deltaT2>-100)&&(amplVar1Cal<limitEnergy2)&&(amplVar1Cal>limitEnergy1)){
...
flag=1;
      ...
      for (unsigned int j=10; j<signal_->size() ; j++) {
     ...
     for (unsigned int j=400; j<700+delay ; j++){
...
flag=1;
   ...
   for (unsigned int j=10; j<signal_->size() ; j++) {
     ...
     for (unsigned int j=400; j<700+delay ; j++){ //pulse inverted
